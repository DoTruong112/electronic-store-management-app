...
public class NhanVienDataBase {
    ...
    public boolean InsertNhanVien(NhanVien nv){
        ...
        String sql=String.format("insert into NhanVien(MaNV,TenNV,GioiTinh,NgaySinh,DiaChi,SDT,ChucVu,AnhNV)"
                + "values('%s',N'%s',N'%s','%s',N'%s','%s',N'%s',N'%s')", nv.getMaNV(),nv.getTenNV(),nv.getGioiTinh()
                ,df.format(nv.getNgaySinh()),nv.getDiaChi(),nv.getSDT(),nv.getChucVu(),nv.getAnhNV());
        try {
        ...
        try{
            ...
            while(rs.next()){
                ...
                NhanVien nv=new NhanVien(rs.getString("MaNV"), rs.getString("TenNV"), rs.getString("GioiTinh"), 
                        date, rs.getString("DiaChi"), rs.getString("SDT"),rs.getString("ChucVu"),rs.getString("AnhNV"));
                nhanViens.add(nv);
            }
            cn.close();
    ...
    public boolean UpdateNhanVien(NhanVien nv){
        ...
        String sql=String.format("Update NhanVien set TenNV =N'%s',GioiTinh =N'%s',NgaySinh='%s'"
                + ",DiaChi=N'%s',SDT='%s',ChucVu=N'%s',AnhNV=N'%s' where MaNV='%s'", nv.getTenNV(),nv.getGioiTinh(),
                df.format(nv.getNgaySinh()),nv.getDiaChi(),nv.getSDT(),nv.getChucVu(),nv.getAnhNV(),nv.getMaNV());
        try {
            PreparedStatement ps=(PreparedStatement)cn.prepareStatement(sql);
            ps.executeUpdate();
            cn.close();
